...
/tcm - TCM (554 docs)
/sandpump - Sand pump intel
/agents - Status')

async def tcm(u: Update, c):
    q = ' '.join(c.args) if c.args else ''
    await u.message.reply_text(f'TCM: {q}
...
Yuchai 260 diesel + sand pump
Ghana/Mali/Nigeria gold mining')

async def agents_cmd(u: Update, c):
    await u.message.reply_text('Agents: TCM ONLINE | Sand Pump ONLINE | Air DEV')
